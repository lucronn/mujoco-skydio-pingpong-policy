...
class JugglePolicy:
    ...
    def act(self, obs):
        ...
        if approaching or t < float(getattr(self, "_punch_until", -1e9)):
            ...
            pg = float(np.clip(getattr(p, "punch_gain", 1.55), 1.25, 1.95))
            coll = pg * hs + 6.0 * max(0.0, 0.25 - float(dv[2]))
            coll = float(np.clip(coll, 1.25 * hs, 2.10 * hs))
            u = self._mix(coll, tip)
            self.last_cmd = np.clip(np.asarray(u, float), self.rot_min, self.rot_max)
            return [float(x) for x in self.last_cmd]
...
def score_contacts(result: dict, cx: float, cy: float) -> dict:
    ...
    # Rank tall loft hard — visual juggle needs mean_vz well above 1.5.
    fit = (succ, mean_vz, (n if crash == 0 else -crash), mean_up, -max_r - chatter_pen, -n_all)
    return {
        "fit": fit,
        "n_vert": n,
        "mean_up": mean_up,
