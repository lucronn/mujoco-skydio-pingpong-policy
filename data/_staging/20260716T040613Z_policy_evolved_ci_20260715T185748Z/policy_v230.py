...
class Policy:
    ...
    def _window_dive_cmd(self, obs, t, dp, dv, gates):
        ...
        vz=float(np.clip(5.0*(z_des-float(dp[2]))-1.3*float(dv[2]), -6.2, 0.3))
    ...
    def act(self,obs):
        try:
            ...
            if self.gate2_eta3 is None and t>=1.14 and bool(obs.get("ball_visible",False)) and float(raw_p[0])>1.0 and float(raw_p[2])>1.5:
                ...
                self.fast_midcourse = bool(self.gate2_eta3 < 0.32 or vx >= 4.40 or (abs(vy)>=1.2 and yerr>0.30))
            ...
            if (not self.g3_smashed) and _race_open and t >= 0.90 and bool(obs.get("ball_visible", False)) and ((not self._skip_smash) or self._arm_ymiss > 1.05 or t >= 1.20):
                ...
                if self._race_g1:
                    ...
                    _g1_lim = float(g1[0]) + (0.40 if (int(self.contact_count) == 0 and y_miss < 0.40) else -0.02)
                else:
