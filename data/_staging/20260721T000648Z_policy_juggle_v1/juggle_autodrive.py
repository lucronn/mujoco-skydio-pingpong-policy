...
class JuggleParams:
    ...
    sep_drop: float = 0.22  # target drop below hit altitude during SEP
...
class JugglePolicy:
    ...
    def act(self, obs):
        ...
        # Permanent park after N bounces — hold still under spawn (readable on close cam).
        if self._parked:
            return _hold_at(float(self.cx), float(self.cy), 1.00, tip_lim=0.10)

        # Proximity-based contact → SEP (partial-obs bvz is too late/noisy for bounce edge).
        dxy = float(math.hypot(float(bp[0]) - float(rp[0]), float(bp[1]) - float(rp[1])))
        dz = float(bp[2] - rp[2])
        contactish = dxy < 0.30 and -0.05 < dz < 0.28
        if contactish and (t - float(self._last_hit_t)) > 0.32 and t > float(self._sep_until):
            self._sep_until = t + max(0.28, float(p.sep_s))
            self._hit_z = float(dp[2])
            self._hit_count += 1
            self._last_hit_t = t
            if self._hit_count >= N_BOUNCE:
                self._want_park = True
            # Drop hard under the ball so loft is visually obvious.
            return _hold_xy_z(float(self._hit_z) - max(0.28, float(p.sep_drop)), tip_scale=0.08)
        ...
        if t < float(self._sep_until):
            z_hold = float(getattr(self, "_hit_z", float(dp[2]))) - max(0.28, float(p.sep_drop))
            return _hold_xy_z(z_hold, tip_scale=0.08)

        # Wait for clear separation + falling ball before remount.
        if float(bv[2]) > -0.05 or dz < 0.32:
            z_hold = min(float(bp[2]) - 0.55, float(getattr(self, "_hit_z", 1.0)) - float(p.sep_drop))
            return _hold_xy_z(z_hold, tip_scale=0.15)

        # Remount under ball with altitude-hold collective (avoid geometric-ctrl tip-flips).
        x_des = 0.45 * float(bp[0]) + 0.55 * float(self.cx)
        y_des = 0.45 * float(bp[1]) + 0.55 * float(self.cy)
        ...
        # Strong upward punch into approaching ball — visible loft, not soft pin.
        if 0.14 < dz < 0.48 and float(bv[2]) < 0.25 and dxy < 0.28:
            tip = np.array([
                float(np.clip(0.8 * (x_des - dp[0]), -0.12, 0.12)),
                float(np.clip(0.8 * (y_des - dp[1]), -0.12, 0.12)),
                0.0,
            ])
            coll = 1.38 * hs + 4.0 * (0.05 - float(dv[2]))
            coll = float(np.clip(coll, 1.15 * hs, 1.70 * hs))
            u = self._mix(coll, tip)
            self.last_cmd = np.clip(np.asarray(u, float), self.rot_min, self.rot_max)
            return [float(x) for x in self.last_cmd]
        return _hold_at(x_des, y_des, z_des, tip_lim=0.16)
...
def score_contacts(result: dict, cx: float, cy: float) -> dict:
    ...
    fit = (succ, (n if crash == 0 else -crash), mean_up + 0.05 * mean_vz, -max_r - chatter_pen, -n_all)
    return {
        "fit": fit,
        "n_vert": n,
        "mean_up": mean_up,
...
def seed_pool(rng: random.Random) -> list:
    seeds = [
        JuggleParams(),
        JuggleParams(dz_under=0.2893, xy_kp=6.0, xy_kd=2.80, z_kp=9.1, z_kd=2.62, tip_flat_y=0.06, face_gain=0.62, blend=0.65, center_x=0.08, sep_s=0.40, sep_coll=1.11, remount_dz=0.13, sep_drop=0.15),
        JuggleParams(dz_under=0.28, sep_s=0.40, sep_drop=0.28),
        JuggleParams(dz_under=0.40, xy_kp=6.5, sep_s=0.38, sep_drop=0.25),
        JuggleParams(dz_under=0.32, z_kp=9.0, sep_s=0.34, sep_drop=0.18),
        JuggleParams(dz_under=0.36, xy_kp=4.5, remount_dz=0.14, sep_s=0.42, sep_drop=0.30),
        JuggleParams(dz_under=0.30, tip_flat_y=-0.4, blend=0.7, sep_s=0.36, sep_coll=1.08),
        JuggleParams(dz_under=0.38, xy_kp=7.0, xy_kd=3.5, sep_coll=1.10, sep_drop=0.24),
        JuggleParams(dz_under=0.25, tip_flat_y=-0.6, face_gain=1.2, z_kp=8.0, sep_s=0.40),
        JuggleParams(dz_under=0.42, tip_flat_y=0.2, max_xy_acc=5.0, remount_dz=0.16, sep_s=0.32),
        JuggleParams(dz_under=0.34, tip_flat_y=0.0, face_gain=0.3, xy_kp=5.5, sep_s=0.45, sep_drop=0.26),
    ]
    while len(seeds) < 16:
        seeds.append(mutate(rng.choice(seeds[:6]), rng))
    return seeds
