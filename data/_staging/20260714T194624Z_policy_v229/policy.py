# policy_v229: v228 antiscrub + L1c band2 tend~arm_ym
...
class Policy:
    ...
    def act(self,obs):
        try:
            ...
            if (not self._early_in) and 0.54 <= t <= 0.58 and bool(obs.get("ball_visible", False)):
                ...
                if _tau > 0.05:
                    ...
                    if _ym >= 1.02 and abs(float(_bv[1])) <= 2.0 and float(_bp[1]) >= 0.26:
            ...
            if (not self._early_in) and 0.58 <= t <= 0.68 and bool(obs.get("ball_visible", False)):
                ...
                if _tau > 0.05:
                    ...
                    if 0.94 <= _ym <= 1.02 and abs(float(_bv[1])) <= 1.60 and float(_bp[1]) >= 0.36:
                        ...
                        self._far=float(np.sign(_bv[1])) if abs(float(_bv[1]))>0.15 else 1.0
            _tend = (0.83 + 0.10*float(min(0.20, max(0.0, getattr(self,"_arm_ymiss",0.95)-0.90))/0.20)) if int(getattr(self, "_early_band", 0)) == 2 else 0.80
            ...
            if (np.isfinite(g0c) and (t - g0c) < 0.085 and (not self.g3_smashed)
                ...
                if y_miss > 0.70 or abs(float(raw_v[1])) > 1.85:
                    self.last_cmd=np.zeros(4); return [0.0, 0.0, 0.0, 0.0]
                if y_miss > 0.45 and (t - g0c) < 0.14:
                    self.last_cmd=np.zeros(4); return [0.0, 0.0, 0.0, 0.0]
            ...
            if (self._skip_smash and (not self.rescue_done[1]) and (not self.g3_smashed)
                ...
                if ready:
                    ...
                    ty = float(np.clip(1.05*far - 0.35*float(bv_r[1]) + 0.15*(g1[1]-bp_r[1]), -1.15, 1.15))
                    self._punch_cmd = self._mix(32.0, np.array([0.20, ty, 0.0]))
            ...
            if (not self.g3_smashed) and (not self.rescue_done[1]) and t >= 0.90 and bool(obs.get("ball_visible", False)) and ((not self._skip_smash) or self._arm_ymiss > 1.05 or t >= 1.20):
                ...
                if y_miss > 0.80 and float(bp_r[0]) > g0x - 0.40:
                    self._race_g1 = True
                if self._race_g1 and float(bp_r[0]) < float(g1[0]) - 0.02:
                    ...
                    if float(bp_r[0]) < meet_x - 0.55:
                        if t - self.last_contact_t < 0.28:
                            self.last_cmd = np.zeros(4); return [0.0, 0.0, 0.0, 0.0]
                        pd = np.array([
                            float(meet_x - 0.25),
                            float(meet_y + far * 0.28),
                    ...
                    ready = (0.22 < dxy_r < 0.62 and -0.05 < dz_r < 0.48 and float(bv_r[2]) < 0.85
                             and float(bp_r[0]) > meet_x - 0.35 and far_off > 0.08)
                    if ready:
                        self.rescue_done[1] = True; self.pop_end_t = t
                        ty = float(np.clip(0.75*far - 0.55*float(bv_r[1]) + 0.40*(g1[1]-bp_r[1]), -1.15, 1.15))
                        self.last_cmd = self._mix(38.0, np.array([0.16, ty, 0.0]))
                        self.sep_until = t + 0.40
                        return [float(x) for x in self.last_cmd]
                    pd = np.array([
                        float(bp_r[0] - 0.08),
