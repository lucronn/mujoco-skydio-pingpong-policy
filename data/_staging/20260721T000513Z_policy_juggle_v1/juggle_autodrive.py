...
class JugglePolicy:
    ...
    def act(self, obs):
        ...
        if contactish and (t - float(self._last_hit_t)) > 0.32 and t > float(self._sep_until):
            self._sep_until = t + float(p.sep_s)
            self._hit_z = float(dp[2])
            self._hit_count += 1
            self._last_hit_t = t
            if self._hit_count >= N_BOUNCE:
                self._want_park = True
            return _hold_xy_z(float(self._hit_z) - float(p.sep_drop), tip_scale=0.10)
        ...
        if t < float(self._sep_until):
            z_hold = float(getattr(self, "_hit_z", float(dp[2]))) - float(p.sep_drop)
            return _hold_xy_z(z_hold, tip_scale=0.10)
