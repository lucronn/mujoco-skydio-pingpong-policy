...
class Policy:
    ...
    def act(self,obs):
        try:
            ...
            # After the deferred impulse, cut open-loop settle and hand back to FSM.
            if (
                lu is not None
                and self.defer_smash
                and t >= float(self.smash_go_t) + 0.085
                ...
                if _ym < 0.45 and float(_rp[0]) > float(_bp[0]) + 0.08:
                    lu=None
            if lu is not None:
            ...
            if (not self.g3_smashed) and _race_open and t >= 0.90 and bool(obs.get("ball_visible", False)) and ((not self._skip_smash) or self._arm_ymiss > 1.05 or t >= 1.20):
                bp_r, bv_r = raw_p, raw_v
                ...
                if float(bp_r[0]) > g0x - 0.40:
                    ...
                    # through looks identical on nom with uncounted contacts during launch).
                        ...
                        self._race_g1 = True
                if self._race_g1:
                    # Centered lag: keep chasing a few ticks past g1 x (013 tips only after bx>g1).
                    _g1_lim = float(g1[0]) + (float(th.get("race_lim_span",0.40)) if (int(self.contact_count) == 0 and y_miss < 0.40) else -0.02)
                else:
                ...
                if self._race_g1 and float(bp_r[0]) < _g1_lim:
                    ...
                    # Second chance: punch fired but no contact — re-loft before ball clears g1.
                    if (
                        self.rescue_done[1]
                        and int(self.contact_count) == 0
                        and (t - float(self.pop_end_t)) > 0.06
                        and (t - float(self.pop_end_t)) < 0.35
                        and float(bp_r[0]) < float(g1[0]) - 0.08
                    ):
                        dxy_r = float(np.linalg.norm((bp_r - rp)[:2])); dz_r = float(bp_r[2] - rp[2])
                        behind_ok = float(rp[0]) <= float(bp_r[0]) + float(th.get("lag_behind_eps", 0.05))
                        if dxy_r < 0.70 and -0.12 < dz_r < 0.55 and behind_ok:
                            ty = float(np.clip(float(th["race_ty_far"])*far - float(th["race_ty_vy"])*float(bv_r[1]) + float(th["race_ty_g"])*(g1[1]-bp_r[1]), -1.15, 1.15))
                            self.last_cmd = self._mix(float(th["race_mix"]) + 6.0, np.array([0.08, ty, 0.08]))
                            self.sep_until = t + 0.10
                            self.pop_end_t = t
                            return [float(x) for x in self.last_cmd]
                    ...
                    # Near-level lag tip: 013 briefly reaches dxy≪0.2 with |ahead|<0.12 but
                    # often slightly above (dz≤0) — tip up before scrubbing from above.
                    if (
                        (not ready)
                        and lag_noc
                        and dxy_r < 0.22
                    ...
                    if ready and (not self.rescue_done[1]):
                        self.rescue_done[1] = True; self.pop_end_t = t
                        ty = float(np.clip(float(th["race_ty_far"])*far - float(th["race_ty_vy"])*float(bv_r[1]) + float(th["race_ty_g"])*(g1[1]-bp_r[1]), -1.15, 1.15))
                        up = float(th.get("lag_up", 0.10)) if lag_noc else 0.0
                        mix_boost = float(th.get("lag_mix_boost", 8.0)) if lag_noc else 0.0
                        self.last_cmd = self._mix(float(th["race_mix"]) + mix_boost, np.array([0.06 if lag_noc else 0.16, ty, up]))
                        self.sep_until = t + (0.03 if lag_noc else float(th["race_sep"]))
                        self._loft_gate = 0
                        return [float(x) for x in self.last_cmd]
                    # Aggressive under-ball chase for lag: hard brake when ahead of ball.
                    if lag_noc:
