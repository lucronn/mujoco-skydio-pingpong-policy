...
class Policy:
    ...
    def act(self,obs):
        try:
            ...
            # force_fast_after_g1: unlock g3 smash for slow midcourse stuck at bg=2
            if (np.isfinite(self.cross_seen_t[1]) and (not bool(self.bounce_done[2]))
                    and (not self.g3_smashed) and t >= 1.22):
            ...
            if self.fast_midcourse:
                ...
                if (not self.g3_smashed) and before_g3 and t>=1.18 and float(bp_u[0]) > float(gates[2,0]) - 1.55:
