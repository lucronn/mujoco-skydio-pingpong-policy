...
class Policy:
    ...
    def reset(self, info=None):
        ...
        # Descent control for gates 3-4
        self.descent_control_active = False
        self.last_bounce_apex = 0.0
        self.post_launch_filter_reset_done = False
    ...
    def _update_ball_filter(self, obs, t, dt):
        ...
        if new_sample:
            ...
            # CRITICAL FIX: After open-loop launch, the filter prior is completely wrong
            # (it predicted free-fall through the ground). Hard-reset to measurement.
            if not self.post_launch_filter_reset_done and t > self.launch_mode_until:
                # Reset filter state to the measured values
                self.ball_p = zp.copy()
                self.ball_v = zv_now.copy()
                self.ball_P = np.diag([0.05, 0.05, 0.08, 0.10, 0.10, 0.16]) ** 2
                self.post_launch_filter_reset_done = True
    ...
    def _gate_required_z(self, gate):
        # Add robustness margin for gate height jitter [0, 0.04]
        return float(gate[2] + 1.90 + self.ball_r + 0.01 + 0.05)
    ...
    def _update_progress(self, t, ball_p, ball_v, racket_p, gates):
        ...
        if dist_xy < 0.34 and abs(dist_z) < 0.12 and (dvz > 0.15 or ball_v[2] > 0.15) and t - self.last_contact_t > 0.06:
            ...
            # Track apex after bounce for energy management
            if ball_v[2] > 0.5:
                # Ball is ascending, estimate apex
                apex_z = ball_p[2] + (ball_v[2]**2) / (2.0 * self.g)
                self.last_bounce_apex = apex_z
    ...
    def _desired_ball_velocity_for_gate(self, ball_p, gate, target_next):
        ...
        # Moderate horizontal velocity for gates 3-4 to allow descent control
        if self.gate_idx >= 2:
            vx = float(np.clip(dx / 0.55, 1.5, 2.8))
        else:
    ...
    def _plan(self, obs, t, drone_p, drone_v, racket_p, ball_p, ball_v, gates, target):
        ...
        # Descent control: if ball is descending too fast after gate 2, brake it
        if ball_v[2] < -2.0 and time_since_contact > 0.15 and self.gate_idx >= 2 and not self.descent_control_active:
            self.descent_control_active = True
        
        if self.descent_control_active and ball_v[2] < -1.0 and self.gate_idx >= 2:
            # Actively brake descent by moving drone upward under ball
            self.phase = "DESCENT_BRAKE"
            pos_des = bp - np.array([0.0, 0.0, self.racket_local_z - 0.05])
            vel_des = np.array([0.3 * desired_ball_v[0], 0.3 * desired_ball_v[1], 1.2], dtype=float)
            if ball_v[2] > -1.5:
                self.descent_control_active = False
    ...
    def _controller(self, obs, pos_des, vel_des, accel_ff, yaw_des):
        ...
        tilt_deg = 50.0 if self.phase in ("INTERCEPT", "BOUNCE", "DESCENT_BRAKE") else 40.0
        ...
        if self.last_cmd is not None:
            alpha = 0.72 if self.phase in ("INTERCEPT", "BOUNCE", "DESCENT_BRAKE") else 0.58
